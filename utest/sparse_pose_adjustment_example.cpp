...
int main() {
  ...
  for (int i = 0; i < N; i++) {
    for (int j = i + 1; j <= i + M; j++) {
      if (j < N) {
        ...
        meas.delta.bottomRows(2) = R.transpose() * (state.col(j).bottomRows(2) -
                                                    state.col(i).bottomRows(2));
  ...
  ceres::CostFunction* cost_function =
      spa_origin_error::Create(Vector3d::Zero());
  problem.AddResidualBlock(cost_function,
                           NULL,  // new CauchyLoss(0.5),
                           pose);
  ...
  for (int i = 0; i < N; i += 10) {
    printf("i: %d, %lf, %lf, %lf\n", i, pose[i * 3 + 0], pose[i * 3 + 1],
           pose[i * 3 + 2]);
    printf("sta: %lf, %lf, %lf\n", state(0, i), state(1, i), state(2, i));
    printf("err: %lf, %lf, %lf\n", state(0, i) - pose[i * 3 + 0],
           state(1, i) - pose[i * 3 + 1], state(2, i) - pose[i * 3 + 2]);
  }
